...
public:
	...
	~Camera();
	const Vector3D get_ray_direction(const int i, const int j, float epsx=0, float epsy=0) const;
	const Vector3D& get_position() const { return position; }
	void drawPixel(int i, int j, Color c, int n);
	float * getBitmap() {return bitmap; }
